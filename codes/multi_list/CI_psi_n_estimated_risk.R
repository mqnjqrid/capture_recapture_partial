...
for(n0 in n_vec){print(n0)
  for(s in 1:100){print(s)
    ...
    estim = estim_multi_risk(List_matrix, K, l, actual = actual, func = func, risk_alpha = risk_alpha, i = 2, j = 1, risk_min = risk_min, risk_max = risk_max, alpha = 0.25, omega = 1, length = length)
...
pdf(paste0("C:/Users/manja/Dropbox/capture_recapture_partial/codes/images/summary_risk_K", K, '_l', l, '_psi0', round(psi0*100), '.pdf'), height = 6.5, width = 15, onefile = FALSE)
#ggarrange(g11, g5, nrow = 1, common.legend = TRUE, legend = "bottom")
ggall
dev.off()

save(pestim, pc, file = paste0("C:/Users/manja/Dropbox/capture_recapture_partial/codes/multi_list/datarisk_K", K, '_l', l, '_psi0', round(psi0*100), '.Rdata'))
